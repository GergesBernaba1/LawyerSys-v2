...
public sealed class AiAssistantOrchestrationService : IAiAssistantOrchestrationService
{
    private readonly LegacyDbContext _context;
    private readonly IAiAssistantTextService _aiTextService;

    public AiAssistantOrchestrationService(LegacyDbContext context, IAiAssistantTextService aiTextService)
    {
        _context = context;
        _aiTextService = aiTextService;
    }

    public async Task<AiSummaryResponseDto> SummarizeAsync(AiSummaryRequestDto request, CancellationToken cancellationToken = default)
    {
        ...
        var systemPrompt = language == "ar"
            ? "أنت مساعد قانوني ثنائي اللغة. قدم تلخيصًا عمليًا للنص بدون اختلاق معلومات."
        ...
        var userPrompt = language == "ar"
            ? $"لخص النص التالي في فقرة قصيرة ثم أضف نقاطًا رئيسية (من 3 إلى {maxPoints})، وكل نقطة تبدأ بـ \"- \".\n\n{request.Text}"
    ...
    public async Task<AiDraftResponseDto> DraftAsync(AiDraftRequestDto request, CancellationToken cancellationToken = default)
    {
        var language = NormalizeLanguage(request.Language);
    ...
    public async Task<AiTaskSuggestionsResponseDto> GetTaskDeadlineSuggestionsAsync(AiTaskSuggestionsQueryDto query, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var task in tasks)
        {
            ...
            if (reminderLeadHours < 12)
            {
                ...
                suggestions.Add(new AiTaskSuggestionItemDto
                {
                    ...
                    Priority = "Medium",
                    Rationale = language == "ar"
                        ? "وقت التذكير الحالي قريب جدًا من موعد التنفيذ. يوصى بتذكير مبكر لتحسين الالتزام."
        ...
        return new AiTaskSuggestionsResponseDto
        {
            Language = language,
