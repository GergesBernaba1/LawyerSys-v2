...
public static class DemoDataSeeder
{
    ...
    private static async Task<SubscriptionPackageSeedResult> EnsureSubscriptionPackagesAsync(ApplicationDbContext applicationDbContext)
    {
        ...
        if (monthly == null)
        {
            ...
            {
                Name = template?.Name ?? "Monthly Plan",
                NameAr = template?.NameAr ?? "الباقة الشهرية",
                Description = template?.Description ?? "Monthly subscription billed every month.",
                DescriptionAr = template?.DescriptionAr ?? "اشتراك شهري يُجدد كل شهر.",
                Feature1 = template?.Feature1 ?? "Case and customer management",
                Feature1Ar = template?.Feature1Ar ?? "إدارة القضايا والعملاء",
                Feature2 = template?.Feature2 ?? "Courts, hearings, and documents",
                Feature2Ar = template?.Feature2Ar ?? "المحاكم والجلسات والمستندات",
                Feature3 = template?.Feature3 ?? "Billing, notifications, and reporting",
                Feature3Ar = template?.Feature3Ar ?? "الفوترة والإشعارات والتقارير",
        ...
        if (annual == null)
        {
            annual = new SubscriptionPackage
            {
                ...
                IsActive = true,
                DisplayOrder = monthly.DisplayOrder + 1,
                CreatedAtUtc = now,
                UpdatedAtUtc = now,
            };
