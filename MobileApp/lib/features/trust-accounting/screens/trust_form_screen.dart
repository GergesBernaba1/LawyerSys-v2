          ...
          child: ListView(
            children: [
              TextFormField(
                ...
                decoration: InputDecoration(labelText: localizer.caseCode ?? 'Case ID'),
                validator: (value) => value == null || value.isEmpty ? localizer.allFieldsAreRequired ?? 'Required' : null,
              ),
              TextFormField(
                controller: _accountController,
                decoration: InputDecoration(labelText: localizer.accountId ?? 'Account ID'),
                validator: (value) => value == null || value.isEmpty ? localizer.allFieldsAreRequired ?? 'Required' : null,
              ),
              TextFormField(
                controller: _typeController,
                decoration: InputDecoration(labelText: localizer.transactionType ?? 'Transaction Type'),
                validator: (value) => value == null || value.isEmpty ? localizer.allFieldsAreRequired ?? 'Required' : null,
              ),
              TextFormField(
                controller: _amountController,
                decoration: InputDecoration(labelText: localizer.amount ?? 'Amount'),
                keyboardType: const TextInputType.numberWithOptions(decimal: true),
                validator: (value) => value == null || value.isEmpty ? localizer.allFieldsAreRequired ?? 'Required' : null,
              ),
              TextFormField(
                controller: _statusController,
                decoration: InputDecoration(labelText: localizer.status ?? 'Status'),
                validator: (value) => value == null || value.isEmpty ? localizer.allFieldsAreRequired ?? 'Required' : null,
              ),
              const SizedBox(height: 16),
              TextButton(onPressed: _pickDate, child: Text('${localizer.dateLabel ?? 'Date'}: ${_selectedDate.toLocal().toIso8601String().split('T').first}')),
              TextFormField(
